...
#include "reflection_get_member_tuple_helper.hpp"
	...
	template <typename T, ::std::size_t Current, ::std::size_t Total, typename AccSeq>
	struct build_serializable_indices_impl {
		using next_seq = ::std::conditional_t<
			is_member_serializable<T, Current>(),
			typename index_sequence_push_back<AccSeq, Current>::type,
			AccSeq
		>;
		using type = typename build_serializable_indices_impl<T, Current + 1, Total, next_seq>::type;
	};
	...
	struct serializable_indices {
		using type = typename build_serializable_indices_impl<
			remove_cvref_t<T>,
			0,
			members_count_v<remove_cvref_t<T>>,
			::std::index_sequence<>
		>::type;
	};
