...
inline constexpr bool is_char_array_v = ::std::is_array_v<remove_cvref_t<T>> &&
                                        is_char_v<::std::remove_extent_t<T>>;
...
inline constexpr bool is_int_v =
    ::std::is_integral_v<remove_cvref_t<T>> && !is_char_v<T> &&
    !is_char_pointer_v<T> && !is_char_array_v<T> && !is_bool_v<T>;

// Check is int64_t
template <typename T>
inline constexpr bool is_int64_v =
    ::std::is_same_v<remove_cvref_t<T>, int64_t> ||
    ::std::is_same_v<uint64_t, remove_cvref_t<T>>;
...
template <typename T>
inline constexpr bool is_ignored_v =
    is_template_instant_of<::tinyrefl::ignore, remove_cvref_t<T>>::value;

// Check is serializable
template <typename T>
inline constexpr bool is_serializable_v =
    !is_ignored_v<T> && !is_smart_pointer_v<T>;
...
inline constexpr bool is_custom_type_v =
    ...
    !is_char_pointer_v<remove_cvref_t<T>> && !is_array_v<remove_cvref_t<T>> &&
    !is_int_v<remove_cvref_t<T>> && !is_int64_v<remove_cvref_t<T>> &&
    !is_floating_v<remove_cvref_t<T>> && !is_bool_v<remove_cvref_t<T>> &&
    !is_ignored_v<remove_cvref_t<T>> && !is_smart_pointer_v<remove_cvref_t<T>>;

template <typename T>
struct sequence_element_type_impl {
  static_assert(
      false, "sequence_element_type just be use in sequence container type!");
};
...
inline constexpr ::std::string_view get_member_name() {
  ...
  constexpr ::std::string_view content =
      funcName.substr(val_pos + 6, end - (val_pos + 6));
  ...
  if constexpr (dot != ::std::string_view::npos) {
    ...
    if constexpr (name.ends_with(");"))
      return name.substr(0, name.size() - 2);
    else if constexpr (name.ends_with(")"))
  ...
  constexpr size_t begin =
      funcName.substr(tmpBegin, tmpEnd - tmpBegin).rfind("::") + 2;
  return funcName.substr(tmpBegin + begin, tmpEnd - tmpBegin - begin);
#endif
}

template <auto index, auto tuple>
inline constexpr ::std::string_view get_member_name_v =
    get_member_name<&::std::get<index>(tuple)>();
