...
TEST_CASE("to_json - multiple ignore fields: correct comma count") {
    ...
    int comma_count = 0;
    for (char ch : json) { if (ch == ',') ++comma_count; }
    CHECK(comma_count == 1);
...
TEST_CASE("from_json - multiple objects of same type independently") {
    ...
    CHECK(a.x == 1); CHECK(a.y == 2);
    CHECK(b.x == 3); CHECK(b.y == 4);
    CHECK(c.x == 5); CHECK(c.y == 6);
}
...
TEST_CASE("from_json - field after nested struct is parsed") {
    const char* json = R"({"before":"B","nested":{"x":10,"y":20},"after":"A","count":99})";
    AfterNested obj{};
...
TEST_CASE("from_json - fields before and after nested struct") {
    ...
    CHECK(obj.mid.x == 2);
    CHECK(obj.mid.y == 3);
    CHECK(obj.after == 4);  // <-- critical: does parent resume after nested EndObject?
}
...
TEST_CASE("from_json - multiple vector fields all parsed") {
    ...
    CHECK(obj.a[0] == 1); CHECK(obj.a[1] == 2);
    REQUIRE(obj.b.size() == 3);
    CHECK(obj.b[0] == "x"); CHECK(obj.b[2] == "z");
...
struct L1 { int v; };
struct L2 { L1 a; L1 b; };
struct L3 { L2 x; string label; };
struct L4 { L3 inner; int count; };
...
TEST_CASE("from_json - nested struct then vector then string field") {
    ...
    CHECK(obj.nums[1] == 8);
    CHECK(obj.nums[2] == 9);
    CHECK(obj.tag == "end");  // <-- must be parsed after both nested obj and array
}
