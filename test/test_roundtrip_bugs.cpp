...
TEST_CASE("roundtrip - double nested struct") {
  DoubleNested obj{{1, {"inner_a", {10, 20}}}, {2, {"inner_b", {30, 40, 50}}}};
  string json;
  tinyrefl::reflection_to_json(obj, json);
...
TEST_CASE("roundtrip - vector of nested structs") {
  HasNestedVec obj{"collection", {{"A", {1, 2}}, {"B", {3, 4, 5}}, {"C", {}}}};
  string json;
  tinyrefl::reflection_to_json(obj, json);
...
TEST_CASE("from_json - extra unknown fields are ignored") {
  const char* json =
      R"({"name":"Alice","age":30,"address":{"city":"SH","zip":200},"unknown_field":"xxx","another":42})";
  Person restored{};
...
TEST_CASE("roundtrip - all basic types in one struct") {
  AllBasic obj{true,        -42,     42u, -9000000000LL, 9000000000ULL, 1.5f,
               2.718281828, "hello", 'X'};
  string json;
  tinyrefl::reflection_to_json(obj, json);
...
TEST_CASE("from_json - int field with float JSON value") {
  // RapidJSON parses 3.5 as double; assign to int should truncate or fail
  // gracefully
...
TEST_CASE("from_json - invalid JSON returns error") {
  const char* bad_json =
      R"({"name": "Alice", age: 30})";  // missing quotes on key
  Person restored{};
