...
TEST_CASE(
    "from_json - uint64_t field from small positive JSON integer (Int event)") {
...
TEST_CASE("roundtrip - ragged 2D vector") {
  ...
  REQUIRE(r.rows[0].size() == 1);
  CHECK(r.rows[0][0] == 1);
  REQUIRE(r.rows[1].size() == 2);
  CHECK(r.rows[1][1] == 3);
  REQUIRE(r.rows[2].size() == 3);
  CHECK(r.rows[2][2] == 6);
  CHECK(r.rows[3].empty());
  REQUIRE(r.rows[4].size() == 4);
  CHECK(r.rows[4][3] == 10);
}
