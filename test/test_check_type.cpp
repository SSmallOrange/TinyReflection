...
TEST_CASE("type traits - is_int") {
  ...
  static_assert(tinyrefl::detail::is_int_v<int32_t>,
                "int32_t should be integral");
  static_assert(!tinyrefl::detail::is_int_v<char>,
                "char should not be integral");
  static_assert(!tinyrefl::detail::is_int_v<const char*>,
                "const char* should not be integral");
  static_assert(!tinyrefl::detail::is_int_v<bool>,
                "bool should not be integral");
  static_assert(tinyrefl::detail::is_int_v<unsigned long long>,
                "unsigned long long should be integral");
  static_assert(!tinyrefl::detail::is_int_v<float>,
                "float should not be integral");
  static_assert(!tinyrefl::detail::is_int_v<double>,
                "double should not be integral");
  static_assert(!tinyrefl::detail::is_int_v<void>,
                "void should not be integral");
  CHECK(true);
}

TEST_CASE("type traits - is_int64") {
  static_assert(!tinyrefl::detail::is_int64_v<int32_t>,
                "int32_t should not match");
  static_assert(!tinyrefl::detail::is_int64_v<long>,
                "long might not match on all platforms");
  static_assert(!tinyrefl::detail::is_int64_v<float>, "float should not match");
  static_assert(!tinyrefl::detail::is_int64_v<void*>,
                "pointer should not match");
  CHECK(true);
}

TEST_CASE("type traits - is_floating") {
  static_assert(tinyrefl::detail::is_floating_v<float>, "float should match");
  static_assert(tinyrefl::detail::is_floating_v<const float>,
                "const float should match");
  static_assert(tinyrefl::detail::is_floating_v<volatile float>,
                "volatile float should match");
  static_assert(!tinyrefl::detail::is_floating_v<int>, "int should not match");
  static_assert(tinyrefl::detail::is_floating_v<float&>,
                "float reference should match");
  static_assert(!tinyrefl::detail::is_floating_v<float*>,
                "float pointer should not match");
  CHECK(true);
}

TEST_CASE("type traits - platform types") {
  static_assert(tinyrefl::detail::is_int_v<size_t>,
                "size_t should be integral");
  static_assert(tinyrefl::detail::is_int_v<std::ptrdiff_t>,
                "ptrdiff_t should be integral");
  static_assert(!tinyrefl::detail::is_floating_v<size_t>,
                "size_t should not be float");
  CHECK(true);
}

TEST_CASE("type traits - cv qualifiers and references") {
  static_assert(tinyrefl::detail::is_int_v<const volatile int&>,
                "cv int& should match");
  static_assert(tinyrefl::detail::is_floating_v<float&>, "float& should match");
  CHECK(true);
}
...
TEST_CASE("type traits - custom types should not match primitives") {
  struct Empty {};
  static_assert(!tinyrefl::detail::is_int_v<Empty>,
                "custom type should not match int");
  static_assert(!tinyrefl::detail::is_floating_v<Empty>,
                "custom type should not match float");
  static_assert(!tinyrefl::detail::is_int64_v<int[10]>,
                "array should not match int64");
  CHECK(true);
}
