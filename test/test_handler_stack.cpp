...
// test struct whose fields come in a specific order that stresses stack
// ordering
struct Stress {
  string s1;
  Point p1;
...
TEST_CASE("roundtrip - vector of structs then scalar field") {
  ...
  CHECK(r.points[0].y == 0);
  CHECK(r.points[1].x == 1);
  CHECK(r.points[1].y == 0);
  CHECK(r.points[2].x == 1);
  CHECK(r.points[2].y == 1);
  CHECK(r.points[3].x == 0);
  CHECK(r.points[3].y == 1);
...
TEST_CASE("roundtrip - graph with source, target, waypoints, weight") {
  Graph g{{1, "src"}, {2, "dst"}, {{3, "w1"}, {4, "w2"}, {5, "w3"}}, 100};
  string json;
  ...
  CHECK(st.ok);
  CHECK(r.source.id == 1);
  CHECK(r.source.label == "src");
  CHECK(r.target.id == 2);
  CHECK(r.target.label == "dst");
  REQUIRE(r.waypoints.size() == 3);
  CHECK(r.waypoints[0].id == 3);
  CHECK(r.waypoints[0].label == "w1");
  CHECK(r.waypoints[1].id == 4);
  CHECK(r.waypoints[1].label == "w2");
  CHECK(r.waypoints[2].id == 5);
  CHECK(r.waypoints[2].label == "w3");
  CHECK(r.weight == 100);
}
...
TEST_CASE("from_json - vector of structs with extra keys in each element") {
  ...
  CHECK(r.points[0].x == 1);
  CHECK(r.points[0].y == 2);
  CHECK(r.points[1].x == 3);
  CHECK(r.points[1].y == 4);
...
TEST_CASE("from_json - overwrite existing non-zero values") {
  Frame f{};
  // Parse once
  CHECK(tinyrefl::reflection_from_json(
            f, R"({"id":1,"origin":{"x":10,"y":20},"data":[1,2,3],"tag":"a"})")
            .ok);
  CHECK(f.id == 1);
  CHECK(f.origin.x == 10);
  ...
  CHECK(tinyrefl::reflection_from_json(
            f, R"({"id":2,"origin":{"x":30,"y":40},"data":[9],"tag":"b"})")
            .ok);
  CHECK(f.id == 2);
  CHECK(f.origin.x == 30);
  CHECK(f.tag == "b");
...
TEST_CASE("from_json - nested struct keys in reverse order") {
  ...
  const char* json =
      R"({"id":5,"origin":{"y":20,"x":10},"data":[7],"tag":"rev"})";
  Frame r{};
  auto st = tinyrefl::reflection_from_json(r, json);
  CHECK(st.ok);
  CHECK(r.id == 5);
