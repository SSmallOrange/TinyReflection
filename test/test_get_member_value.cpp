...
void run_reflection_tests() {
    ...
    {
        Test1 t1{42};
        auto& ref = tinyrefl::struct_member_reference_by_index<0>(t1);
    ...
    {
        Test2 t2{3.14, 2.71};
        auto& ref_x = tinyrefl::struct_member_reference_by_index<0>(t2);
        auto& ref_y = tinyrefl::struct_member_reference_by_index<1>(t2);
    ...
    // 测试3: 三个成员结构体（边界情况）
    {
        ...
        auto& name_ref = tinyrefl::struct_member_reference_by_index<0>(t3);
        auto& age_ref = tinyrefl::struct_member_reference_by_index<1>(t3);
        auto& active_ref = tinyrefl::struct_member_reference_by_index<2>(t3);
    ...
    // 测试4: 四个成员结构体
    {
        ...
        auto c_ref = tinyrefl::struct_member_reference_by_index<0>(t4);
        auto s_ref = tinyrefl::struct_member_reference_by_index<1>(t4);
        auto i_ref = tinyrefl::struct_member_reference_by_index<2>(t4);
        auto l_ref = tinyrefl::struct_member_reference_by_index<3>(t4);
    ...
    // 测试5: 五个成员结构体（边界情况）
    {
        ...
        auto f1_ref = tinyrefl::struct_member_reference_by_index<0>(t5);
        auto f5_ref = tinyrefl::struct_member_reference_by_index<4>(t5);
    ...
    // 测试6: const 对象
    {
        ...
        const auto& name_ref = tinyrefl::struct_member_reference_by_index<0>(const_t3);
        const auto& age_ref = tinyrefl::struct_member_reference_by_index<1>(const_t3);
    ...
    // 测试7: 临时对象（右值）
    {
        auto&& name_ref = tinyrefl::struct_member_reference_by_index<0>(Test3{"Dave", 50, false});
    ...
    // 测试8: 成员类型验证
    {
        ...
        static_assert(std::is_same_v<
            decltype(tinyrefl::struct_member_reference_by_index<0>(t3)),
            std::string&
        >);
        static_assert(!std::is_same_v<
            decltype(tinyrefl::struct_member_reference_by_index<1>(Test3{})),
            int&&
        >);
        static_assert(std::is_same_v<
            decltype(tinyrefl::struct_member_reference_by_index<2>(t3)),
            bool&
        >);
    ...
    // 测试9: 索引边界检查（编译时）
    {
        // 应该触发静态断言
        // Test2 t2;
        // auto& invalid_ref = tinyrefl::struct_member_reference_by_index<2>(t2); // 应失败
    ...
    // 测试10: 不同值类别
    {
        ...
        // 左值引用
        auto& lv_ref = tinyrefl::struct_member_reference_by_index<0>(t2);
        lv_ref = 10.0;
        assert(t2.x == 10.0);
        
        // 右值引用
        auto&& rv_ref = tinyrefl::struct_member_reference_by_index<1>(std::move(t2));
        rv_ref = 20.0;
        assert(t2.y == 20.0);
